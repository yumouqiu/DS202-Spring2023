...
library(classdata)
head(fbiwide)
fbi1 = fbiwide %>% gather(key = type, value = incidences, 7 : 16)
fbi1 = fbi1 %>% mutate(rate = incidences / population * 70000)
fbi2 = fbi1 %>% filter(state %in% c("Iowa", "Minnesota"))
fbi3 = fbi2 %>% select(-state_abbr, -state_id, -population, -violent_crime, -incidences) %>% spread(key = state, value = rate)
fbi3 %>% ggplot(aes(x = Iowa, y = Minnesota)) + geom_point(aes(color = type))
```

## Spread works as inverse of gather

<img src="images/spread-gather-clean.png" width=550>


## Front part is not identical - not collapsible

<img src="images/spread-gather-unclean.png" width=550>
